...
static const unsigned short liftUpTime = 1800;
static const unsigned short liftDownTime = 1300;
//static const short swingTime = 890;
static short power;
static const unsigned short rampLineUpDistance = 30;
...
static const float irDistances[4] = {14.5, 10, 17.5, 10}; //do not change this list without updating irBackDistances[]
...
task main(){
  ...
  while (!matchHasStarted){wait1Msec(1);}
	...
	for (short currentCrate = 0; currentCrate < 3; currentCrate++){//runs through the next block four times or until it finds the beacon
		while (distanceTravelled() <= irDistances[currentCrate]-1){move(-50);}//waits until the robot is at each bucket
		if (testing)
			brake(5);
		if (currentCrate == 2)
			brake(2);
	...
	if (!foundCrate || crateVal == 0)//runs through if the sensor did not find the beacon
		crateVal = 3;//the robot is at the fourth bucket if it did not find the beacon anywhere else
