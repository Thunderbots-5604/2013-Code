...
// 1st search and raise lift
  ClearTimer(T1);
  e = a;
  while ((abs(nMotorEncoder[Left]) < (dist1+42*in)) && ir != 1)
  {
  	move(-e);
    if (time1[T1] < b)
  	{
...
// select bucket
  ...
  if ( (abs(nMotorEncoder[Left]) > dist1+(9.5*in)) && (abs(nMotorEncoder[Left]) < dist1+(24*in)) )
  	d = pos2;
  if ( (abs(nMotorEncoder[Left]) > dist1+(24*in)) && (abs(nMotorEncoder[Left]) < dist1+(41*in)) )//previosly 38.5
    d = pos3;
  if ( (abs(nMotorEncoder[Left]) > dist1+(41*in)) )//previosly 38.5
  	d = pos4;
...
// move infront of ramp
  resetEncoders();
  while (abs(nMotorEncoder[Left]) < 26*in)
  {
  	move(40);
  	if (time1[T1] < c)
  	{
