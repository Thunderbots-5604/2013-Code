...
#pragma config(Motor,  mtr_S1_C2_1,     Lift1,       4517
85tmotorTetrix, openLoop, reversed)
...
//Up ramp time
static const short rampLineUpDistance[4] = {25, 25, 27, 25};
static const int UPRAMPTIME = 2000;
static const int UPRAMPDIST[4] = {27, 32, 27, 27};
...
//initializing all the global floats that are used9*
...
task main(){
  ...
  nxtDisplayCenteredTextLine(3, "%2.2f delay set", delay);
	...
	resetEncoders();
	move(60);
	while (distanceTravelled() < rampLineUpDistance[crateVal-1]){move(60);}
	brake();
	...
	resetEncoders();
	moveForward(-100);
	while(distanceTravelled() <= UPRAMPDIST[crateVal-1] && time1[T4] <= UPRAMPTIME){wait1Msec(1);}
/*
	brake();
	wait1Msec(100);
	swingLeft(false);
...
void turn45(bool right){ //true is right, false is left
	...
	motor[Right2] = -power;
	//wait1Msec(TURN45TIME);
	while ((abs(nMotorEncoder[Left]) + abs(nMotorEncoder[Right]))/2 < 650)
