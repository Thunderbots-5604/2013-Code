...
float correctInputs(int iinput){
	float input = (float) iinput;
	if (abs(input) < 20)
		input = 0;
	if (input > 0)
	{
...
task main()
{
  ...
  while(true){
  	...
  	//movement
		if (!joy2Btn(12)){
  		setMovementPower(correctInputs(joystick.joy1_x1), correctInputs(joystick.joy1_y2));
		} else {
			setMovementPower(correctInputs(joystick.joy2_x1), correctInputs(joystick.joy2_y2));
		}
