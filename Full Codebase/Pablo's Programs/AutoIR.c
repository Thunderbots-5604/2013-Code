...
task main()
{
  ...
  float deg = 16.5;
  int ir = 0;
  int f = -1;//facing backward
  float a = 36;//1st distance is facing backward. HAS TO BE ADJUSTED!

  if (SensorValue[sonar] < 50)
  {
  	f = 1;//facing foward
  	a = 36;//1st distance is facing foward. HAS TO BE ADJUSTED!

  }
	//beacon 1
  //line up with beacon 1
  nMotorEncoder[Left] = 0;
  while(abs(nMotorEncoder[Left]) < a*in)
  {
  	motor[Left] = f*100;
   	motor[Right] = f*100;
  }
  ...
  //align with goal
  if (f == -1)//facing bakwards
  {
    //goal 3 and 4 are already aligned
    if (ir == 1 || ir == 2)//align 1 or 2
    {
      nMotorEncoder[Left] = 0;
      while(abs(nMotorEncoder[Left]) < 2*in)//same direction as robot
      {
        motor[Left] = -70;
        motor[Right] = -70;
      }
    ...
    {
      nMotorEncoder[Left] = 0;
      while(abs(nMotorEncoder[Left]) < 39*in)
      {
        motor[Left] = 100;
        motor[Right] = 100;
      }
